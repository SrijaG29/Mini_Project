...
function getdetails(event){
    ...
    let currentMonth = monthNames[currentDate.getMonth()]; // Months are 0-indexed, so add 1
    let currentYear = currentDate.getFullYear();

    // Create the expense object with month and year
    let d = {
...
function monthwisecontent(categoryName,totalAmount) {
    let monthWiseContentDetails = document.getElementById("monthwisedetails");
    let listItems = monthWiseContentDetails.getElementsByTagName("li");
    let found = false;
    for(let li of listItems){
        let [currentCategory, currentAmount] = li.textContent.split(": ₹");
        // console.log(currentAmount.type);
        currentAmount = parseInt(currentAmount);

        if(currentCategory === categoryName){
            currentAmount = currentAmount + parseInt(totalAmount);
    ...
    if(!found){
        ...
        li.textContent = `${categoryName}: ₹${parseInt(totalAmount)}`;
        li.setAttribute("class","listItems");
        monthWiseContentDetails.appendChild(li);
        categoryTotals[categoryName] = parseInt(totalAmount);
    }

}

let monthfilter = document.getElementById("monthDropdown");
let yearfilter = document.getElementById("yearDropdown");
function filterByMonthAndYear() {
    let monthWiseContentDetails = document.getElementById("monthwisedetails");
    monthWiseContentDetails.innerHTML = ""; // Clear the list before adding new items

    let ili = document.createElement("li");
    ili.innerHTML = "<b>Total Expense<b>";
    ili.setAttribute("class","listItems");
    monthWiseContentDetails.appendChild(ili);
    ...
    Object.values(details).forEach(value => {
        if (value.month == selectedMonth && value.year == selectedYear) {
            monthwisecontent(value.category, value.totalAmount);
            count++;
        }
    });
    if(count===0){
...
monthfilter.onchange = filterByMonthAndYear;
yearfilter.onchange = filterByMonthAndYear;
